...
export default {
  ...
  methods: {
      ...
      if (children && children.length) {
        children.forEach(child => {
          ...
          if (this.allowCheckedChildrenOfDisabledChild) {
            ...
            if (child.isParent) {
              this.$set(child, 'check_Child_State', 2)
            } else {
              // 无子节点
              this.$set(child, 'check_Child_State', -1)
            }
          }
        })
      }
    ...
    parentCheckedHandle(parentNode, checked, halfcheck = false) {
      ...
      if (halfcheck) {
        ...
        if (oldHalfCheck !== nodeHalfcheck) {
          ...
          this.$set(parentNode, 'check_Child_State', 1)
        } else if (nodeHalfcheck) {
          return false
        }
        // 当前节点全部选中
        if (allBortherNodeChecked) {
          this.$set(parentNode, 'check_Child_State', 2)
        }
    ...
    nodeSelected(node, position) {
      ...
      if (node.checked) {
        if (node.isParent) {
          this.$set(node, 'check_Child_State', 2)
        } else {
          this.$set(node, 'check_Child_State', -1)
        }
      } else {
        if (node.isParent) {
          this.$set(node, 'check_Child_State', 0)
        } else {
          this.$set(node, 'check_Child_State', -1)
        }
      }
