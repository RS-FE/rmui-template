  ...
  <div class="tree-container">
    <div class="tag-box-container">
      <div class="tag-box" ref="txtbox" @click="open = !open" @mouseleave="leaveTextTag">
        ...
        <div class="tag" @click.stop="tagClick($event)" ref="txttag" v-for="(node, idx) in selectedItems" :key="idx">
          {{ node }}
...
.tree-container .tag-box-container {
  position: relative;
  width: 100%;
  height: 36px;
  overflow: hidden;
...
.tree-container .tree-box ul {
  margin-left: 0;
  -webkit-padding-start: 10px;
}

.tree-container .tree-container .search-input {
  width: 96%;
  height: 30px;
...
.tree-container .blank {
  background-color: #fff;
  border: 0;
}
.tree-container .rmNode {
  position: absolute;
  right: 5px;
  top: 5px;
  width: 15px;
...
.tree-container .fade-enter-active,
.tree-container .fade-leave-active {
  transition: opacity 0.5s, transform 0.5s;
  /*transition: all 0.5s;*/
}

.tree-container .fade-enter,
.tree-container .fade-leave-active {
  opacity: 0;
